...
function libFileName() {
  ...
  if (process.platform === "win32") return `query-engine-embedded-windows-${arch2}.dll`;
  if (process.platform === "darwin") return `query-engine-embedded-macos-${arch2}.dylib`;
  return `query-engine-embedded-linux-${arch2}.so`;
}
function findLibrary() {
  const candidates = [
    path5.resolve(process.cwd(), ".dbcube", "bin", libFileName()),
    path5.resolve(process.cwd(), "node_modules", ".dbcube", "bin", libFileName())
  ];
...
function loadNative() {
  ...
  try {
    ...
    const take = (ptr) => {
      if (!ptr) throw new Error("embedded engine returned null");
      const s = koffi.decode(ptr, "char", -1);
      cFree(ptr);
      return s;
    };
    const asyncCall = (fn, ...args) => new Promise((resolve6, reject) => {
    ...
    native = {
      connect: (cfg) => asyncCall(cConnect, cfg),
      execute: (handle, dml, txId) => asyncCall(cExecute, handle, dml, txId),
      raw: (handle, query, params, txId) => asyncCall(cRaw, handle, query, params, txId),
      begin: (handle) => asyncCall(cBegin, handle),
      commit: (handle, txId) => asyncCall(cCommit, handle, txId),
      rollback: (handle, txId) => asyncCall(cRollback, handle, txId),
      disconnect: (handle) => asyncCall(cDisconnect, handle)
    };
    return native;
  } catch {
    loadFailed = true;
...
var EmbeddedEngine = class {
  /** true si la librería nativa está disponible en esta instalación. */
  static available() {
    return loadNative() !== null;
  }
  /** Conecta (una vez por connectionId) y devuelve el handle nativo. */
  static async handleFor(connectionId, config) {
    ...
    const promise = (async () => {
      ...
      if (!lib) throw new Error("embedded engine not available");
      ...
      const res = parseResponse(await lib.connect(JSON.stringify(cfg)));
      if (res.status !== 200 || !res.data?.handle) {
        throw new Error(String(res.message || "embedded connect failed"));
      }
      const handle = Number(res.data.handle);
  ...
  static async executeDml(connectionId, config, dml, txId) {
    ...
    return parseResponse(await lib.execute(handle, JSON.stringify(dml), txId ?? null));
  }
  static async rawQuery(connectionId, config, query, params, txId) {
    const lib = loadNative();
    const handle = await this.handleFor(connectionId, config);
    return parseResponse(await lib.raw(handle, query, JSON.stringify(params ?? []), txId ?? null));
  }
  static async begin(connectionId, config) {
    const lib = loadNative();
    const handle = await this.handleFor(connectionId, config);
    return parseResponse(await lib.begin(handle));
  }
  static async commit(connectionId, config, txId) {
    const lib = loadNative();
    const handle = await this.handleFor(connectionId, config);
    return parseResponse(await lib.commit(handle, txId));
  }
  static async rollback(connectionId, config, txId) {
    const lib = loadNative();
    const handle = await this.handleFor(connectionId, config);
    return parseResponse(await lib.rollback(handle, txId));
  }
